...
/*   Updated: 2023/11/09 00:00:58 by otamrani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int get_pixel_color(t_data *data, int x, int y, int ntext)
{
    ...
    //     return (0);
    pixel = data->imgs[ntext]->img_data + (y * data->imgs[ntext]->len + x * (data->imgs[ntext]->bits / 8));
    // printf("x= %d y = %d\n",x , y);
    ...
    data->wall_height = ((900 / 10) / data->distance) * data->dist_proj_plane;
     y_start = (900 / 2) - (data->wall_height / 2);
        ...
        if (data->wall_height > 900) {
            m = 900;
            // j = 1;
            y_img = (data->wall_height - 900) / 2 / data->wall_height * data->imgs[data->Nimg]->height;
        } else
            m = (900 / 2) + (data->wall_height / 2);
         while (y_start < m) {
            if (data->check == 1) {
                x_img = ((data->wall_hit_y / (900 / 7) ) - floor(data->wall_hit_y / (900 / 7))) * data->imgs[data->Nimg]->width;
            } else if (data->check == 0)
                x_img = ((data->wall_hit_x / (1300 / 17)) - floor(data->wall_hit_x / (1300 / 17))) * data->imgs[data->Nimg]->width;
            dst = data->addr + (y_start * data->line_length + i * (data->bits_per_pixel / 8));
            if (y_img >= data->imgs[data->Nimg]->height)
                y_img = data->imgs[data->Nimg]->height - 1;
            if (x_img >= data->imgs[data->Nimg]->width)
                x_img = data->imgs[data->Nimg]->width - 1;
            *(unsigned int *)dst = get_pixel_color(data, (int)x_img, (int)y_img, data->Nimg);
            y_img += data->imgs[data->Nimg]->height / data->wall_height;
            y_start++;
        }
       i++;
    }
